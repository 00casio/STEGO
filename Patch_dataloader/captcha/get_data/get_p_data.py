        ...
        for patch_file,label_file in tqdm(zip(patch_file_list, mask_file_list), total=len(patch_file_list)):
            ...
            if saving_path:
                ...
                for n, (single_patch, single_label) in enumerate(zip(patch, label)):
                    ...
                    single_patch_normalized = ((single_patch - single_patch.min()) / (single_patch.max() - single_patch.min()) * 255).astype(np.uint8)
                    patch_img = Image.fromarray(single_patch_normalized, mode="L")  

                    #patch_im   g.save(os.path.join(saving_path_patch, f"{patch_file.replace('.npy', '')}_{n}.png"))
                    img_name = patch_file.split('/')[-1].replace('.npy', '') + f"_{n}"

                    label_name = label_file.split('/')[-1].replace('.npy', '') + f"_{n}"
                    label_img = Image.fromarray(255*single_label.astype(np.uint8), mode="L")
                    np.save(os.path.join(saving_path_label, label_name),label_img)
                    np.save(os.path.join(saving_path_patch,img_name),patch_img)
                                        
                    label_name = label_file.split('/')[-1].replace('.npy', '') + f"_{n}.png"
                    img_name = patch_file.split('/')[-1].replace('.npy', '') + f"_{n}.png"

                    # Save images as PNG
                    label_img.save(os.path.join(saving_path_label, label_name))
                    patch_img.save(os.path.join(saving_path_patch, img_name))
